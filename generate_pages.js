...
const code = fs.readFileSync(codeHtmlPath, 'utf-8');

const headerPart = code.split('<!-- BEGIN: HeroHeader -->')[0];
const header = headerPart.replace(/href="#/g, 'href="code.html#');

const footerPart = code.split('<!-- BEGIN: Footer -->')[1];
const footer = '  <!-- BEGIN: Footer -->' + footerPart;

const privacy = `  <!-- BEGIN: Privacy Policy -->
...
const terms = `  <!-- BEGIN: Terms of Use -->
...
fs.writeFileSync(privacidadeHtmlPath, header + '\\n' + privacy + '\\n' + footer, 'utf-8');
fs.writeFileSync(termosHtmlPath, header + '\\n' + terms + '\\n' + footer, 'utf-8');

console.log("LGPD Pages HTML generated cleanly.");
